...
| 2026-08-01 | Experimental `claude-workflow` target: compiles the phase graph into `.claude/workflows/*.js` (loops/gates as real control flow, `agentType` reuse of `.claude/agents/`). Opt-in only — excluded from `--target all` (paid-plan + Claude Code only). Spike report: `docs/research/spike-claude-workflows-target.md`. | claude-code | Phase graph is what a workflow script encodes; keeps intermediate results out of the context window |
